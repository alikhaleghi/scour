...
impl Yahoo {
    pub fn name(&self) -> &'static str { "yahoo" }
        ...
        for element in document.select(&result_selector) {
            if let Some(title_el) = element.select(&title_selector).next() {
                ...
                if url.contains("RU=") && url.contains("/RK=") {
                    if let Some(start) = url.find("RU=") {
                        ...
                        if let Some(end) = sub.find("/RK=") {
                            if let Ok(decoded) = urlencoding::decode(&sub[..end]) { url = decoded.into_owned(); }
                        }
                    }
                }
                ...
                if !url.is_empty() && !title.is_empty() {
                    results.push(SearchResultItem { title, url, snippet, engine: self.name().to_string() });
                }
            }
        }
        Ok(results)
